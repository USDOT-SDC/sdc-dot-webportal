export const environment = {
    production: true,
    REGION: 'us-east-1',
    USER_POOL_ID: 'us-east-1_e13lOvIqH',
    CLIENT_ID: '6cuv84i9t3lprgu28kt5h2hi53',
    IDENTITY_PROVIDER: 'prod-dot-ad',
    APP_DOMAIN: 'ecs-prod-sdc-dot-webportal',
    API_ENDPOINT: 'webportal-api/',
    LOGIN_GOV_COGNITO_APP_CLIENT_ID: '6tbelkt0fgbv3icatfc1ojbsbc',
    ACCOUNT_LINK_URL: 'account-link-prod',
    LINK_ACCOUNT_PATH: 'prod-link-account-private',
    ACCOUNT_LINKED_PATH: 'prod-account-linked-private',
    RESET_TEMPORARY_PASSWORD_PATH: 'prod-reset-temporary-password-private',
  };
  